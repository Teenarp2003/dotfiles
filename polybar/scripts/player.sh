...
artist_name=$(playerctl metadata artist --no-messages)
artist_name_len=$(playerctl metadata artist --no-messages | wc -c)
artist_name_mod=$(playerctl metadata artist --no-messages| head -n1 | awk '{print $1;}')
